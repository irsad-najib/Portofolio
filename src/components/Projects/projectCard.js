...
const ProjectCard = memo(({ project, index, isVisible }) => {
  ...
  return (
    <>
      ...
      {isModalOpen && (
        <div
          className={`fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 transition-all duration-300 ease-out ${modalVisible ? "opacity-100" : "opacity-0"}`}>
          <div
            className={`absolute inset-0 bg-black/80 backdrop-blur-sm transition-opacity duration-300 ${modalVisible ? "opacity-100" : "opacity-0"}`}
            onClick={closeModal}
          />

          <div
            className={`relative bg-zinc-900/95 border border-cyan-400/20 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden transition-all duration-300 ease-out ${modalVisible ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 translate-y-4"}`}>
            <div className="flex justify-between items-center p-4 sm:p-6 border-b border-cyan-400/20">
              <div>
                ...
                <div
                  className={`inline-block px-2 py-1 rounded text-xs font-semibold ${project.status === "Completed" ? "bg-green-400/20 text-green-400 border border-green-400/30" : "bg-yellow-400/20 text-yellow-400 border border-yellow-400/30"}`}>
                  {project.status}
                </div>
              </div>
              <button
